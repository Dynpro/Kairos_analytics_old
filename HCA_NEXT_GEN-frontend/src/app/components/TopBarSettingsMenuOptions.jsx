...
//   if (userPermissions?.referral && userPermissions['referral'] === 1)
//     arr.push(settingOptions['referral']);
...
function generateSettingOptions(userPermissions, userRole) {
  const arr = [];

  // Super Admin (role_id: 2) gets all settings options
  if (userRole === 2 || userRole === 'SA') {
    arr.push(
      settingOptions['clients'],
      settingOptions['users'],
      settingOptions['roles'],
      settingOptions['groups'],
      settingOptions['user_access'],
      settingOptions['looker'],
      settingOptions['download_dashboard'],
      settingOptions['reports'],
      settingOptions['generate_reports'],
      settingOptions['invite_user'],
      settingOptions['referral'],
      settingOptions['about']
    );
    return arr;
  }

  // Download Dashboard is available to all authenticated users
  arr.push(settingOptions['download_dashboard']);
...
const TopBarSettingsMenuOptions = ({ togglePopup }) => {
  const { uaPermissions, userRole } = useSelector((state) => ({
    uaPermissions: state.userAccessPermissions.userPermissions,
    userRole: state.userDetails?.user?.role,
  }));
  ...
  const settingsOptions = generateSettingOptions(uaPermissions, userRole);
  const navigate = useNavigate();
  const dispatch = useDispatch();

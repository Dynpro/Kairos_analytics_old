...
$router->group(['prefix' => 'api'], function () use ($router) {
    ...
    $router->group(['middleware' => 'auth'], function () use ($router) {
        ...
        //Navigation – all routes now served from StudioDashboardController
        // (user-access filtered endpoints)
        $router->get('/clientCateWiseUserAccess', 'LookerDataController@getUserAccessClientsCategoryWise');
        $router->get('/clientCateWiseUserAccessNew', 'StudioDashboardController@getUserAccessClients');
        $router->get('/getUserAccessClients', 'StudioDashboardController@getUserAccessClientsFlat');
        $router->get('/getUserAccessClientsNew', 'StudioDashboardController@getUserAccessClientsFlat');
        $router->get('/getUserAccessFolders', 'StudioDashboardController@getUserAccessFolders');
        $router->get('/getUserAccessdashboards', 'StudioDashboardController@getUserAccessDashboards');

        // All-dashboards endpoints (no user filter, for admin/allClients screens)
        $router->get('/getLookerClientCategoryWise', 'LookerDataController@getLookerClientCategoryWise');
        $router->get('/getLookerClientCategoryWiseNew', 'StudioDashboardController@getClientCategoryWiseNew');
        $router->get('/getLookerClient', 'StudioDashboardController@getClientsAll');
        $router->get('/getLookerClientFolder', 'StudioDashboardController@getFoldersAll');
        $router->get('/getLookerFolderDashboard', 'StudioDashboardController@getDashboardsAll');

        // Studio dashboard endpoints
        $router->post('/getStudioDashboard', 'LookerDataController@getStudioDashboard');
        $router->post('getDashboard', 'StudioDashboardController@getDashboard');

        // Studio dashboard admin CRUD
        $router->get('/studio_dashboards', 'StudioDashboardController@index');
        $router->post('/studio_dashboards', 'StudioDashboardController@store');
        $router->put('/studio_dashboards/{id}', 'StudioDashboardController@update');
        $router->delete('/studio_dashboards/{id}', 'StudioDashboardController@destroy');
        $router->post('/studio_dashboards/access', 'StudioDashboardController@assignAccess');
        $router->post('/studio_dashboards/bulk', 'StudioDashboardController@bulkUpsert');
        $router->post('/studio_dashboards/superadmin', 'StudioDashboardController@grantSuperAdminAccess');
        $router->get('/studio_dashboards_folder', 'StudioDashboardController@getStudioFolderDashboard');
        $router->post('/studio-pdf-download', 'StudioDashboardController@downloadPdf');
        $router->post('/studio-share-dashboards', 'StudioDashboardController@shareAllDashboards');
       ...
       $router->get('phm_automation_report_list/{id}', 'PHMAutomationController@show_one');
       $router->put('phm_automation_report_list/{id}', 'PHMAutomationController@update');
       ...
       $router->post('phm_automation_store/retry/{id}', 'PHMAutomationController@retry');
       ...
       $router->get('studio_phm_template', 'PHMAutomationController@get_template');
       $router->post('studio_phm_template', 'PHMAutomationController@save_template');
       ...
       //Localization
        ...
        $router->get('gen_chart_data','GenerateReportController@gen_chart_data');
        $router->get('down__pdf','GenerateReportController@down__pdf');
        $router->post('/report/update_flag','GenerateReportController@update_flag');

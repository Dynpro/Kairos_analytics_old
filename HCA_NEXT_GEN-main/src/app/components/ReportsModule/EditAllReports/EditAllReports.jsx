...
const EditAllReports = () => {
  ...
  const getPhmReport = async () => {
    try {
      const response = await axios.get(commonConfig.urls.getphm + '/' + location.state.id, {
        headers: {
          Authorization: `Bearer ${authToken}`,
          'Content-Type': 'application/json',
        },
      ...
      if (response && response.data.Status === 'Success') {
        const reportData = response.data.Response.phm[0];
        const reportDetailsData = {
          reportName: reportData.name || '',
          clientName: reportData.client_id || '',
          reportType: reportData.report_type || '',
          medStartDate: reportData.start_date || '',
          medEndDate: reportData.end_date || '',
          pharmaStartDate: reportData.pharma_start_date || '',
          pharmaEndDate: reportData.pharma_end_date || '',
        };

        if (JSON.stringify(reportDetailsData) !== JSON.stringify(reportDetails)) {
          setReportDetails(reportDetailsData);
        }

        const mappedSections = response.data.Response.sections.map((section) => ({
          id: section.id,
          title: section.section_title,
          description: section.section_text,
          sectionno: section.section_no,
          subsections: section.subsections.map((sub) => ({
            id: sub.id,
            title: sub.sub_section_title,
            description: sub.sub_section_text,
            looks: sub.look_img_url
              ? [
                  {
                    image_embed_url: sub.look_img_url,
                    id: sub.look_id,
                    lookName: sub.look_name,
                  },
                ]
              : [],
          })),
        ...
        handleClientSelect(reportData.client_id);
      }
    } catch (error) {
  ...
  const handleSectionDescriptionChange = (sectionId, newDescription) => {
    setSections((prevSections) => {
      return prevSections.map((section) =>
        section.id === sectionId
          ? { ...section, description: newDescription } // Only update the section that changed
          : section
      );
    });
  };

  const handleSubsectionDescriptionChange = (sectionId, subId, newDescription) => {
    setSections((prevSections) => {
      return prevSections.map((section) =>
        section.id === sectionId
          ? {
              ...section,
              subsections: section.subsections.map((sub) =>
                sub.id === subId ? { ...sub, description: newDescription } : sub
              ),
            }
          : section
      );
        ...
        section.id === sectionId
          ? {
              ...
              subsections: [
                ...section.subsections,
                {
                  id: `${section.sectionno}.${section.subsections.length + 1}`, // Auto-increment subsection number
                  title: '',
                  description: '',
                },
              ],
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const response = await axios.put(
        commonConfig.urls.updatephm + '/' + location.state.id,
        fullReportData,
        {
      ...
      <Box sx={{ mb: 4 }}>
        <Box sx={{ display: 'flex', gap: 2, mb: 2 }}>
          ...
          <TextField
            ...
            fullWidth
            label="Client Name"
            value={reportDetails.clientName} // Will store the client ID
            onChange={(e) => handleClientSelect(e.target.value)} // Call the handler with client ID
          >
          ...
          <AccordionDetails>
            <Box sx={{ mb: 2 }}>
              ...
              <ReactQuill
                ...
                onChange={(value) => {
                  handleSectionDescriptionChange(section.id, DOMPurify.sanitize(value));
                }}
              ...
              <Accordion sx={{ mt: 2 }} key={sub.id}>
                ...
                >
                  <Typography style={{ color: 'Orange' }}>
                    <b>
                      Sub-Section {section.sectionno}.{subIndex + 1}
                    </b>
                  </Typography>
                  <Box sx={{ ml: 'auto' }}>
                ...
                <AccordionDetails>
                  ...
                  <Box>
                    {sub.looks && sub.looks.length > 0 ? (
                      <Box>
                        <Typography variant="body2" color="text.secondary">
                          Look Added: {sub.looks[sub.looks.length - 1].look_name}
                        </Typography>
                        <Button
                          variant="outlined"
                          color="error"
